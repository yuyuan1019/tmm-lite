...
1. **设置页** — 填入 TMDB API Key（如在 docker-compose 中已设置环境变量则可跳过）；字幕下载建议至少配置 ASSRT Token 或 OpenSubtitles API Key，SubDL API Key 可作为备用源；默认搜索简体中文（`zh-cn`）
...
字幕源与 TMDB 共用设置页中的网络代理。OpenSubtitles 使用程序内置浏览器 User-Agent，无需手动配置；接口鉴权、网络或限流错误会在任务日志中显示为“字幕下载失败”，不会再误报成“未找到字幕”。
