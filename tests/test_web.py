...
def test_transcode_route_rejects_remote_library(tmp_path: Path) -> None:
    app = create_app(data_dir=tmp_path / "data", start_scheduler=False)
    with TestClient(app) as test_client:
        item_id = _add_remote_stream_item(test_client)
        # Mark the library as remote — the helper leaves connection_type unset.
        with test_client.app.state.session_factory.begin() as session:
            from app.database import Library

            lib = session.query(Library).one()
            lib.connection_type = "ssh"
            session.commit()
        response = test_client.get(f"/api/transcode/{item_id}")

    assert response.status_code == 400
    assert "远程媒体库" in response.json()["error"]

def test_transcode_route_requires_ffmpeg(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    app = create_app(data_dir=tmp_path / "data", start_scheduler=False)
    monkeypatch.setattr("app.main.shutil.which", lambda _name: None)
    with TestClient(app) as test_client:
        item_id = _add_local_stream_item(test_client, tmp_path / "media", b"x" * 64)
        response = test_client.get(f"/api/transcode/{item_id}")
...
def test_transcode_route_streams_fmp4_for_local_mkv(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A local .mkv gets transcoded to a playable fMP4 stream (skipped if no ffmpeg)."""
    import shutil
    import subprocess
    ...
    # Generate a tiny real MKV (H.264 video) with ffmpeg.
    media_root = tmp_path / "media"
    media_root.mkdir(parents=True, exist_ok=True)
    src = media_root / "sample.mkv"
    ...
    app = create_app(data_dir=tmp_path / "data", start_scheduler=False)
    with TestClient(app) as test_client:
        item_id = _add_local_stream_item(test_client, media_root, b"x")
        # Point the item at the .mkv instead of the helper's .mp4.
        with test_client.app.state.session_factory.begin() as session:
            from app.database import MediaItem

            item = session.get(MediaItem, item_id)
            assert item is not None
            item.folder_path = str(src)
            session.commit()
        response = test_client.get(f"/api/transcode/{item_id}?start=0")

    assert response.status_code == 200
    assert response.headers["content-type"].startswith("video/mp4")
    assert response.headers["x-transcode"] == "ffmpeg"
    # fMP4 starts with an 'ftyp' box and contains moov fragments.
    assert response.content[:4] == b"\x00\x00\x00\x18" or b"ftyp" in response.content[:16]
    assert b"moov" in response.content
